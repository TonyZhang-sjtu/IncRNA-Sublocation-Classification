...
def main():
    ...
    if args.cv:
        ...
        # 调用超参数选择函数
        best_n_estimators, results = select_n_estimators(args.training_file, args.test_file, args.k, n_estimators_range, cv=5, scoring='accuracy', learning_rate=args.learning_rate, max_depth=args.max_depth)

        # 打印最佳结果
        print(f"Optimal n_estimators: {best_n_estimators}")

        # 调用分类函数
        classification_with_xgboost(
            args.training_file,
            args.test_file,
            k=args.k,
